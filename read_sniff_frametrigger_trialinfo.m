function [Sniff,frametrigger,data,Sniff_time]=read_sniff_frametrigger_trialinfo(h5,path_used,pre,post,inh_detect)
...
for i=1:numel(Keys)
    %i
    data2=h5read(h5,strcat(Keys{i},'/Events'));
...
%Concatenate frametrigger
frametrigger=cell2mat(frametrigger(cellfun(@(x) ~isempty(x),frametrigger)));
...
%Remove duplicate
if length(frametrigger)~=length(unique(frametrigger))
    fprintf('duplicated frametrigger')
end
frametrigger2 = sort(unique(frametrigger));%If there is no duplicates, frametrigger2=frametrigger;
...
sniffall_time=[-29999:0,1:length(sniff_all),length(sniff_all)+1:length(sniff_all)+30000];
sniff_all=[int16(zeros(30000,1));sniff_all;int16(zeros(30000,1))];
...
% figure;
% subplot(211);plot(Sniff(:,401:1100)');
% subplot(212);plot(Sniff2(:,401:1100)');
