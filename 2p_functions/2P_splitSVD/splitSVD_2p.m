...
%     num_trials=numel(frames);
    ...
    if ndims(frames{1})==3
        mask = true(size(frames{1},1), size(frames{1},2));
...
for i=1:num_block
    ...
    num_f = size(subF,2);
    [Ub,Sb,~] = svd(subF,'econ');
    G{i}=Ub(:,1:min(num_svals,num_f))*Sb(1:min(num_svals,num_f),1:min(num_svals,num_f));
...
U=U(:,1:min(num_svals,size(U,2)));
SV=SV(:,1:min(num_svals,size(SV,2)));
