...
[~,inh_trig]=min(s_var(971:1030));
inh_trig=inh_trig+970;
pre=inh_trig-1;
...
for i=trial
    %Changed in 3/1/2018
    %     SF(i,:)=MyButterworth(Sniff(i,:),1000,2,[0.001,50]);
    
    %detrend first to prevent y-shift
    %Modified 10/11/2019
    ds = detrend(Sniff(i,:));
    trend = Sniff(i,:)-ds;
    %     SF(i,:)=MyButterworth(ds,1000,2,[0.001,50])+trend;
    
    %Some rigs have discontinuous trend due to needle valves
    %Use median filtered signal to subtract discontinous baseline.
    tmp = MyButterworth(ds,1000,2,[0.001,50])+trend;
    
    
    %Corrected 012520 medfilt1(tmp - medfilt1(tmp,750),30) loose sharp
    %peaks
...
for tr=trial
    clear position inh_onset inh_offset inh_dur sniff_dur
    
    %worked in 1p
    %     [~,~,peaks_exh,peaks_inh] = peakdet(SF(tr,:)-mean(SF(tr,:)), 100, 'th', 40);
    %worked in 2p
    sf = SF(tr,:)-mean(SF(tr,:));
    [~,~,peaks_exh,peaks_inh] = peakdet((sf/std(sf)), 0.5, 'th', 40);
    ...
    if ~isempty(peaks_exh)&&~isempty(peaks_inh)
        ...
        try
            ...
            FS.first_sniff{tr}=Sniff(tr,position(ind_first,1):position(ind_first,3));
            
        catch
            ffprintf('error in calculating sniff features trial %d',tr)
        end
